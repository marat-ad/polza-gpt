...
5. Верни ПОЛНОЕ отформатированное сообщение для Telegram используя MarkdownV2 форматирование
6. Максимум результатов: ${maxResults}${requestShowAll ? ' (пользователь запросил показать всех)' : ''}
7. Если найдено больше 5 экспертов (в обычном режиме): начни с "Найдено много совпадений, вот топ-5"
8. Если совпадений нет: "Совпадений не найдено. Попробуйте переформулировать запрос." + ближайшие варианты если возможно
9. ОБЯЗАТЕЛЬНО используй MarkdownV2 синтаксис: *Жирный текст* для полей. ВАЖНО: Экранируй специальные символы: \_ \* \[ \] \( \) \~ \` \> \# \+ \- \= \| \{ \} \. \!
